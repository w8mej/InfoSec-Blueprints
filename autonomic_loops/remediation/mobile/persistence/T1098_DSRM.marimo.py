...
def bluf(mo):
    mo.md(
        r"""
        # 目录服务还原模式（DSRM）帐户上的密码更改
        
        > [!CAUTION]
        > **HIGH SEVERITY** | **ASO PLAYBOOK**
        >
        > 目录服务还原模式（DSRM）帐户是域控制器上的本地管理员帐户。攻击者可以更改密码以获得持久性。
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1307444", label="Incident ID", disabled=True)
