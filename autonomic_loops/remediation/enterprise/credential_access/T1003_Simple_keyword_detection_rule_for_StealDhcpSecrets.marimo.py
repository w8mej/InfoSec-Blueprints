...
def bluf(mo):
    mo.md(
        r"""
        # Simple keyword detection rule for StealDhcpSecrets
        
        > [!CAUTION]
        > **MEDIUM SEVERITY** | **ASO PLAYBOOK**
        >
        > Detects interesting keywords based on StealDhcpSecrets tool
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="50005885", label="Incident ID", disabled=True)
...
def context(IncidentContext, alert_confidence, incident_id, target_client, ConfidenceThresholdConfig):
    INCIDENT = IncidentContext(
        ...
        case_id          = "INC-2026-50005885",
        event_source     = "browser_rum",
