...
def bluf(mo):
    mo.md(
        r"""
        # Credential Manager Accessed
        
        > [!CAUTION]
        > **LOW SEVERITY** | **ASO PLAYBOOK**
        >
        > A process has read credentials in the Credential Manager. There will be many false positives so check if the Process ID (PID) is that of known malware on the system.
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1302536", label="Incident ID", disabled=True)
