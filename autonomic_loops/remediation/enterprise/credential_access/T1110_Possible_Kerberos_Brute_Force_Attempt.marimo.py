...
def bluf(mo):
    mo.md(
        ...
        >
        > Detects when hundreds of Kerberos requests are made by a single client. Determine if this is a) normal client and b) normal pattern for that client. This Sigma query is designed to accompany the Corelight Threat Hunting Guide, which can be found here: https://www3.corelight.com/corelights-introductory-guide-to-threat-hunting-with-zeek-bro-logs
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="103555", label="Incident ID", disabled=True)
