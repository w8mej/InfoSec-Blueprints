...
def bluf(mo):
    mo.md(
        ...
        >
        > Detects NTLM requests that originate internally and communicate with an external IP address. Tools such as responder can be used to capture NTLM hashes, etc for offline cracking. This Sigma query is designed to accompany the Corelight Threat Hunting Guide, which can be found here: https://www3.corelight.com/corelights-introductory-guide-to-threat-hunting-with-zeek-bro-logs
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="103768", label="Incident ID", disabled=True)
