...
def bluf(mo):
    mo.md(
        r"""
        # 在属性'SamAccountName'中使用'$'的新用户帐户或重命名用户帐户
        
        > [!CAUTION]
        > **HIGH SEVERITY** | **ASO PLAYBOOK**
        >
        > 通过异常的用户帐户名检测可能的旁路EDR和SIEM。
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1306296", label="Incident ID", disabled=True)
