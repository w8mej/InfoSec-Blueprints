...
def bluf(mo):
    mo.md(
        r"""
        # Workspace Login Marked Suspicious
        
        > [!CAUTION]
        > **LOW SEVERITY** | **ASO PLAYBOOK**
        >
        > Identifies when a workspace login is marked suspicious
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1200107", label="Incident ID", disabled=True)
...
def context(IncidentContext, alert_confidence, incident_id, target_client, ConfidenceThresholdConfig):
    INCIDENT = IncidentContext(
        ...
        case_id          = "INC-2026-1200107",
        event_source     = "google_workspace",
