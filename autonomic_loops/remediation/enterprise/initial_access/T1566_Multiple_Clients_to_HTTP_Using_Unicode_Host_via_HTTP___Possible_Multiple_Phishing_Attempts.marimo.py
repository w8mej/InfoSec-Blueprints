...
def bluf(mo):
    mo.md(
        ...
        >
        > Detects when multiple HTTP requests were made to a single domain that has non-ascii characters(unicode/punycode) and a POST or PUT method was used. This Sigma query is designed to accompany the Corelight Threat Hunting Guide, which can be found here: https://www3.corelight.com/corelights-introductory-guide-to-threat-hunting-with-zeek-bro-logs
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="108937", label="Incident ID", disabled=True)
