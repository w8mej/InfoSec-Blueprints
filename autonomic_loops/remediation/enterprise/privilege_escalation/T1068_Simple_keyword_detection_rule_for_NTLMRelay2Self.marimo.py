...
def bluf(mo):
    mo.md(
        r"""
        # Simple keyword detection rule for NTLMRelay2Self
        
        > [!CAUTION]
        > **MEDIUM SEVERITY** | **ASO PLAYBOOK**
        >
        > Detects interesting keywords based on NTLMRelay2Self tool
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="50008458", label="Incident ID", disabled=True)
...
def context(IncidentContext, alert_confidence, incident_id, target_client, ConfidenceThresholdConfig):
    INCIDENT = IncidentContext(
        ...
        case_id          = "INC-2026-50008458",
        event_source     = "feature_flags",
