...
def bluf(mo):
    mo.md(
        ...
        > [!CAUTION]
        > **SIGMA RULE SEVERITY LEVEL. REFLECTS THE RISK ASSOCIATED WITH A TRUE POSITIVE EVENT. SEVERITY** | **ASO PLAYBOOK**
        >
        > Detects when a single source is connecting to many different SMB or file shares. This indicate the device is scanning the network for open file shares or is connecting and exploiting multiple file shares. Determine if this is normal activity for this source IP. This Sigma query is designed to accompany the Corelight Threat Hunting Guide, which can be found here: https://www3.corelight.com/corelights-introductory-guide-to-threat-hunting-with-zeek-bro-logs
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="103129", label="Incident ID", disabled=True)
...
def context(IncidentContext, alert_confidence, incident_id, target_client, ConfidenceThresholdConfig):
    INCIDENT = IncidentContext(
        ...
        case_id          = "INC-2026-103129",
        event_source     = "zeek",
        affected_client  = target_client.value,
        severity         = "Sigma rule severity level. Reflects the risk associated with a true positive event.",
