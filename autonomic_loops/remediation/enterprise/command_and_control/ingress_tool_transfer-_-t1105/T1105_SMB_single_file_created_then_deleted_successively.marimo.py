...
def bluf(mo):
    mo.md(
        ...
        >
        > Detects if a single file is created, then deleted, then created again. This is a common scenario when replacing a legimate file temporarily to do malicious activities. This Sigma query is designed to accompany the Corelight Threat Hunting Guide, which can be found here: https://www3.corelight.com/corelights-introductory-guide-to-threat-hunting-with-zeek-bro-logs
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="107259", label="Incident ID", disabled=True)
