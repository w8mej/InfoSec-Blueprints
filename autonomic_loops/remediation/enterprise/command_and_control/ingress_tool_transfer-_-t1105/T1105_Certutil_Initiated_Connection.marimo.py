...
def bluf(mo):
    mo.md(
        r"""
        # Certutil Initiated Connection
        
        > [!CAUTION]
        > **HIGH SEVERITY** | **ASO PLAYBOOK**
        >
        > Detects a network connection intitiated by the certutil.exe tool. Attackers can abuse `certutil.exe` to download malware or offensive security tools.
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1301695", label="Incident ID", disabled=True)
