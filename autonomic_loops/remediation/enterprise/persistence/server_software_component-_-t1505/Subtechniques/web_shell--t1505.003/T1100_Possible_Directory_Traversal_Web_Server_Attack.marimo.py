...
def bluf(mo):
    mo.md(
        ...
        > [!CAUTION]
        > **HIGH SEVERITY** | **ASO PLAYBOOK**
        >
        > This rule looks for many variations of a directory traversal attack on a webserver. It includes character encodings that make common queries/detections bypassed. Additionally, it includes characters valid for windows and linux webservers. This Sigma query is designed to accompany the Corelight Threat Hunting Guide, which can be found here: https://www3.corelight.com/corelights-introductory-guide-to-threat-hunting-with-zeek-bro-logs
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="100753", label="Incident ID", disabled=True)
