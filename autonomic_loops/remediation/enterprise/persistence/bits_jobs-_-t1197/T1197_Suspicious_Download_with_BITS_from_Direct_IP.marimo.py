...
def bluf(mo):
    mo.md(
        r"""
        # Suspicious Download with BITS from Direct IP
        
        > [!CAUTION]
        > **HIGH SEVERITY** | **ASO PLAYBOOK**
        >
        > Detects a suspicious download using the BITS client from a direct IP. Adversaries may abuse BITS jobs to persistently execute or clean up after malicious payloads.
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1302515", label="Incident ID", disabled=True)
