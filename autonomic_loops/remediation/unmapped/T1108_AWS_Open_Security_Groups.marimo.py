...
def bluf(mo):
    mo.md(
        r"""
        # AWS Open Security Groups
        
        > [!CAUTION]
        > **SIGMA RULE SEVERITY LEVEL. REFLECTS THE RISK ASSOCIATED WITH A TRUE POSITIVE EVENT. SEVERITY** | **ASO PLAYBOOK**
        >
        > We look for rules allowing inbound traffic on port 22 from any IPs. Then we look for the associated instance IDs and append them to the list.
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="50006565", label="Incident ID", disabled=True)
