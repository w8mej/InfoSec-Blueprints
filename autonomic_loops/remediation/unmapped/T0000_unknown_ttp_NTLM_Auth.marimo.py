...
def bluf(mo):
    mo.md(
        ...
        >
        > Logged when NTLM authentication is used usually for local accounts but NTLM can also be used with domain accounts. The original event title says it is only generated on domain controllers but that is not true. This also gets logged on clients.
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1303215", label="Incident ID", disabled=True)
