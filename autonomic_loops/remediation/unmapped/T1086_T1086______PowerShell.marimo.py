...
def bluf(mo):
    mo.md(
        r"""
        # T1086  非交互式PowerShell
        
        > [!CAUTION]
        > **CRITICAL SEVERITY** | **ASO PLAYBOOK**
        >
        > 通过将powershell.exe以explorer.exe作为父级，来检测非交互式PowerShell活动。
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1306432", label="Incident ID", disabled=True)
