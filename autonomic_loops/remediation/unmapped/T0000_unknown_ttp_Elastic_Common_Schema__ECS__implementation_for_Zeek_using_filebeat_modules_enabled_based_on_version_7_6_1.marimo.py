...
def bluf(mo):
    mo.md(
        r"""
        # Elastic Common Schema (ECS) implementation for Zeek using filebeat modules enabled based on version 7.6.1
        
        > [!CAUTION]
        > **SIGMA RULE SEVERITY LEVEL. REFLECTS THE RISK ASSOCIATED WITH A TRUE POSITIVE EVENT. SEVERITY** | **ASO PLAYBOOK**
        >
        > No description provided. Populate with a concise summary of the detection objective and operational scope.
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="50002654", label="Incident ID", disabled=True)
...
def context(IncidentContext, alert_confidence, incident_id, target_client, ConfidenceThresholdConfig):
    INCIDENT = IncidentContext(
        ...
        case_id          = "INC-2026-50002654",
        event_source     = "detection_response",
