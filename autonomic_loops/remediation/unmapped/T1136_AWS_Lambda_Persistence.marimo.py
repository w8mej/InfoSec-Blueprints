...
def bluf(mo):
    mo.md(
        ...
        >
        > A lambda function executing a suspicious action by, a. Creating an access key to backdoor an IAM user b. Updating a security group to allow ingress on a port c. Updating an assume role policy to allow external access.
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="909826", label="Incident ID", disabled=True)
...
def context(IncidentContext, alert_confidence, incident_id, target_client, ConfidenceThresholdConfig):
    INCIDENT = IncidentContext(
        ...
        case_id          = "INC-2026-909826",
        event_source     = "kubernetes_audit",
