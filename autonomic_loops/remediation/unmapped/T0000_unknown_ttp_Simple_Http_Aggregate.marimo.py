...
def bluf(mo):
    mo.md(
        r"""
        # Simple Http Aggregate
        
        > [!CAUTION]
        > **SIGMA RULE SEVERITY LEVEL. REFLECTS THE RISK ASSOCIATED WITH A TRUE POSITIVE EVENT. SEVERITY** | **ASO PLAYBOOK**
        >
        > Detects a download from Webdav service which could be used as a way to transfer tools internally. Determine if this is a) Legitimate server b) legitimate binary/file
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="107686", label="Incident ID", disabled=True)
