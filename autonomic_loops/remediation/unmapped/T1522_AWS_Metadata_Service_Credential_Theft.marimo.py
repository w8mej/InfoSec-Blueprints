...
def bluf(mo):
    mo.md(
        r"""
        # AWS Metadata Service Credential Theft
        
        > [!CAUTION]
        > **SIGMA RULE SEVERITY LEVEL. REFLECTS THE RISK ASSOCIATED WITH A TRUE POSITIVE EVENT. SEVERITY** | **ASO PLAYBOOK**
        >
        > Detects attempts to query the credential metadata service in load balancer logs that returned a 200 status code.
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="50007574", label="Incident ID", disabled=True)
