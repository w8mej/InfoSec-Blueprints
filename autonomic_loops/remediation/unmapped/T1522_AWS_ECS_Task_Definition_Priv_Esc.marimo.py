...
def bluf(mo):
    mo.md(
        r"""
        # AWS ECS Task Definition Priv Esc
        
        > [!CAUTION]
        > **SIGMA RULE SEVERITY LEVEL. REFLECTS THE RISK ASSOCIATED WITH A TRUE POSITIVE EVENT. SEVERITY** | **ASO PLAYBOOK**
        >
        > Task definitions can be used to potentially escalate privileges in an AWS environment by stealing credentials from running containers using the ECS Task Metadata Service.
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="50003829", label="Incident ID", disabled=True)
...
def context(IncidentContext, alert_confidence, incident_id, target_client, ConfidenceThresholdConfig):
    INCIDENT = IncidentContext(
        ...
        case_id          = "INC-2026-50003829",
        event_source     = "stream_processing",
