...
def bluf(mo):
    mo.md(
        r"""
        # Logon (Interactive) *Creds in memory*
        
        > [!CAUTION]
        > **INFORMATIONAL SEVERITY** | **ASO PLAYBOOK**
        >
        > Type 2 Interactive logons can be dangerous as the credentials (plaintext or hashed passwords) are stored in memory (lsass process) and can be stolen by tools like mimikatz.
        >
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1304439", label="Incident ID", disabled=True)
