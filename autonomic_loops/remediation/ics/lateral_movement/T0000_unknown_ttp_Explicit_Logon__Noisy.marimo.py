...
def bluf(mo):
    mo.md(
        r"""
        # Explicit Logon (Noisy)
        
        > [!CAUTION]
        > **INFORMATIONAL SEVERITY** | **ASO PLAYBOOK**
        >
        > (From ultimatewindowsecurity.com)
        This log is generated when a user connects to a server or runs a program locally using alternate credentials. This includes mapping a drive with different credentials or using 'Run as...' via shift-control-right-clicking a shortcut. It also occurs when a process logs on as a different account, such as the Scheduled Tasks service starting a task under a specific user. Additionally, with User Account Control (UAC) enabled, it triggers when an end user runs a program requiring admin privileges (often associated with consent.exe). Finally, logging on interactively to a server with a domain account generates this event, typically alongside two 4624 events.
        
        >
        > **Immediate Action**: Automated Host Isolation
...
def params(mo):
    ...
    incident_id = mo.ui.text(value="1308287", label="Incident ID", disabled=True)
